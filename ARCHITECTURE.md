...
Every tool installs to the home screen as its own app, and all of it is derived from the registry —
no per-tool PWA edits, ever.

- **Per-tool manifest** — `src/pages/manifest/[slug].webmanifest.ts` emits a per-tool `scope` and
  `start_url`, seeded `theme_color`/`background_color`, PNG `any` + `maskable` icons and a scalable
  SVG.
- **Icons** — composed from `src/lib/icons/` (a category-colour-seeded gradient plus a family/slug
  glyph; see `src/lib/icons/tool-icon.ts`). The scalable SVG is a build-time endpoint
  ...
  so rerun `icons:generate` after adding a tool or changing a glyph or colour.
- **Install button** — `src/tools/_shared/InstallButton.astro`, auto-included by `ToolLayout` and
  mobile-only: native `beforeinstallprompt` one-tap install on Android, with a platform-aware manual
  sheet (Chrome ⋮ menu vs iOS Safari share) as the fallback. **Control its visibility with CSS
