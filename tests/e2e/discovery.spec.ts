...
test.describe('homepage index', () => {
  ...
  test('the directory still covers every tool with collapsed tool groups', async ({ page }) => {
    ...
    // 120 tools − 61 grouped-collapse = 59 directory links (generator, physics, applied-math,
    // date/time, scientific-calculator, shell-quote-escalator and the 5 design tools stay
    // ungrouped; physics has 11 manifest-driven simulators and applied-math five wave-2 tools
    // beside unit-circle).
    await expect(directory.locator('.dir-link')).toHaveCount(59);
  });
