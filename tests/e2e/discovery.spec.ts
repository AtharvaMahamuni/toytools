...
test.describe('homepage directory', () => {
  test('renders five category columns with collapsed tool groups', async ({ page }) => {
    ...
    // 62 tools, with every tool group collapsed to a single entry:
    //   −6 case converters (7→1), −2 JSON tools (3→1), −1 JSON↔YAML (2→1), −1 JSON↔CSV (2→1),
    //   −7 text cleanup (8→1), −5 encoders (6→1), −4 hash generators (5→1).
    // 62 − 26 = 36 directory links (the 5 finance tools are not grouped).
    await expect(directory.locator('.dir-link')).toHaveCount(36);
  });
...
test.describe('category pages', () => {
  test('text-utilities groups into three titled sections', async ({ page }) => {
    ...
    // Grouped rows render mode chips: 7 case converters + 8 text-cleanup tools.
    await expect(page.locator('.cat-chip')).toHaveCount(15);
